// src/components/fieldOperator/OperatorProfilePanel.jsx
import React, { useState } from 'react';
import api from "../../api/api";

export default function OperatorProfilePanel({ userId, role, userProfile, onClose }) {
  const [editing, setEditing] = useState(false);
  const [name, setName] = useState(userProfile?.full_name || 'Operator');
  ...
  const stats = [
    { label: 'Role',       val: role ?? 'Operator', cls: 'lime' },
    { label: 'Session',    val: 'Active',          cls: 'green' },
    { label: 'Access',     val: 'Operator Mode',   cls: 'green' },
  ...
  return (
    <div className="wd-panel" style={{ '--panel-icon-bg': 'rgba(143,220,0,0.08)', '--panel-icon-border': 'rgba(143,220,0,0.25)' }}>
      <div className="wd-panel-header">
        ...
        <div>
          <div className="wd-panel-title">Operator Profile</div>
      ...
      <div className="wd-panel-body">
        ...
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          ...
          <div className="wd-profile-role">{role ?? 'Operator'}</div>
